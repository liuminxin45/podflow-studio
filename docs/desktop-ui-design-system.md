# PodFlow Studio 桌面端 UI 设计系统

状态：**当前规范**

适用范围：Electron / React 工作台

`src/index.css`、`src/App.tsx` 和 `scripts/verify_ui_system.js` 是可执行来源。本文件解释
设计意图和贡献规则，不使用手工更新时间表示实现新旧。

## 设计基线

PodFlow Studio 是高密度的创作工作台。视觉语言参考 OpenHuman 的轻量
白色桌面：常驻单一导航、纯白工作面、极浅冷灰反馈和克制的分隔线。界面层级应由
布局、留白、色彩和位置建立，而不是反复使用粗体、重阴影或高对比色块。

- 默认采用 **400** 字重；普通标签、导航与次级操作采用 **500**。
- 区块标题、列表标题和强语义文本采用 **500**。
- 页面标题和唯一的主操作采用 **550**；不使用 600、650、700、750、800
  作为常规界面强调。
- 选中态使用浅灰底色或细描边；不要同时叠加重描边、阴影和位移动画。
- 主画布和主要工作区使用纯白；辅助栏也优先纯白，仅用边线区隔。
- 容器优先使用白底、`1px` 冷灰细描边和 `6–12px` 圆角；常规容器不使用阴影。
- 全局只使用黑、白与中性灰阶；成功、警告、错误和信息状态也不引入彩色。
- 状态依靠文案、图标形状、字重、边框和明度层级表达，不能只依赖颜色。

## 令牌

`src/index.css` 的 **OpenHuman light system** 和 `src/App.tsx` 的
Ant Design `ConfigProvider` 是运行时令牌的唯一来源。新增界面必须使用
这些变量或对应的 Ant Design token，不能另建一套颜色、圆角或阴影体系。

| 类别 | 令牌 | 值 / 用途 |
| --- | --- | --- |
| 画布 | `--bg-primary` | `#ffffff`，纯白桌面底色 |
| 容器 | `--bg-secondary` | `#fafafa`，仅用于弱分组 |
| 悬停 | `--bg-tertiary` | `#f4f4f5`，冷灰弱反馈 |
| 选中 | `--accent-light` | `#f0f0f1`，非破坏性选中态 |
| 文本 | `--text-primary` | `#202124`，主内容 |
| 次文本 | `--text-secondary` | `#62646a`，说明与次要信息 |
| 边框 | `--border-color` | `#e5e5e7`，常规分隔 |
| 活动边框 | `--border-active` | `#c7c8cc`，焦点/选中辅助反馈 |
| 主操作 | `--accent-primary` | `#242529`，唯一主按钮与关键状态 |
| 成功 | `--success-color` | `#4f5156`，搭配完成图标与明确文案 |
| 警告/信息 | `--warning-color` / `--info-color` | `#62646a`，搭配对应图标与说明 |
| 错误 | `--error-color` | `#303136`，通过更深对比和确认流程强调 |
| 圆角 | `--radius-sm/md/lg` | `6px / 8px / 12px` |
| 阴影 | `--shadow-soft` | 仅极弱层次；普通容器为 `none` |

## 组件规则

### 导航与列表

- `workflow-sidebar` 是唯一全局导航，统一承载今日简报、节目库、当前节目的
  五阶段进度、节目级操作和设置；禁止再叠加第二条产品导航轨道。
- 内容位于独立白色工作面；不得让页面级组件重新绘制另一套应用外壳。
- 每行只保留一个主要状态：浅色背景、细边框或左侧指示条三选一。
- 图标按钮必须有 `title` 和 `aria-label`；有明确目的的入口优先采用
  “左图标 + 右文字”的导航项。
- 完成、警告等状态用小尺寸灰阶标签表达，不能把整行染色。

### 页面布局契约

- 今日简报：默认首页只保留一个主生成动作；生成中显示用户任务阶段，生成后先展示
  播放与质量状态，再提供制作工作台和发布入口。
- 节目库：单一内容画布；页头、筛选和节目表格共享同一内容宽度。
- 素材发现：采集工具 / 素材列表 / 新闻详情。左右栏属于工具面板，不是导航。
- 本期整理：新闻清单 / 故事编辑器 / 资料面板。编辑器始终是视觉主区域。
- 成稿：本期信息 / 节目结构 / 正文编辑。正文编辑获得最大宽度与独立滚动。
- 声音制作：稿件段落 / 节目装配 / 制作设置。装配区是主工作区。
- 发布：成品摘要 / 本地交付，不增加平台导航或智能发布入口。
- 设置：分类导航 / 设置内容，但仍保留唯一全局 `workflow-sidebar`。
- 全局播放器：只占内容工作面底部，不覆盖全局侧栏。

所有阶段页共用 `StageHeader` 的高度、标题字阶、返回/前进位置和主操作
规则。页面内部导航只能切换当前页面的资料或工具，不能重复今日简报、节目库、
阶段和设置入口。

### 按钮、表单与卡片

- 普通按钮字重 500，主按钮 550；同一视觉区域最多一个主按钮。
- 输入控件高度为 `32px`，紧凑尺寸为 `28px`。
- Card、Modal、Drawer、Popover 使用细边框，无投影；不要把每一块信息
  都包装成卡片。

### 动效与可访问性

- 悬停反馈只改变背景或边框；不使用上浮、缩放或脉冲作为常规反馈。
- 遵守 `prefers-reduced-motion`，动效必须可被降级。
- 不能只靠颜色表达状态；保留文字、图标或形状提示。

## 防漂移流程

1. 新增/修改 UI 前，先阅读本文件和 `src/index.css` 的 OpenHuman light
   system。
2. 使用现有令牌；若确需新增令牌，同时更新本文件、`src/index.css`、
   `src/App.tsx` 的 Ant Design token，以及守卫脚本。
3. 不要在 OpenHuman light system 之后追加样式规则；需扩展时直接修改该
   区块，使其始终保有最终覆盖权。
4. 提交前运行 `npm run verify:ui-system`，再运行 `npm run build`。
5. 涉及交互流程的页面，再运行 `npm run acceptance:cdp`。

`npm run verify:offline` 会自动包含 UI 设计系统守卫。守卫验证令牌、
层级规则、无障碍降级规则和这份文档的存在，避免样式基线被无意移除。
