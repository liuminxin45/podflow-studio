...
PodFlow Studio 支持 AI 驱动的全流程自动化，但自动化不得以牺牲内容质量为代价，只允许通过质量门禁的精品内容进入正式发布。
这里的“精品”不是营销口号，而是可验证的生产约束：

- 自动化不得绕过来源核验、事实检查、稿件门禁、发音预检、音频质量检查或人工终审。
- 不得用无来源内容、重复模板、mock 音频、未审核产物或虚构指标填充正式节目。
- 任一质量门禁失败时必须保留真实原因并阻断正式发布，不得静默降级或把失败包装成成功。
- 新增 CLI、桌面端、工作流和发布能力时，必须默认继承当前质量契约，不得提供隐式降质开关。
- “只输出精品内容”表示产物通过明确的机器门禁和人工终审，不代表可以虚构评价、播放量或绝对质量承诺。
