    ...
    try:
        ...
        try:
            final_path, duration_seconds = _assemble_with_pydub(
                readable, output_path, requested_format, config, operations, bgm_path
            )
        except Exception as pydub_error:
            if config.add_bgm:
                raise RuntimeError(f"BGM mixing requires a working pydub/ffmpeg path: {pydub_error}") from pydub_error
            degraded = True
