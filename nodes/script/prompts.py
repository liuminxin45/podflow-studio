...
- 不直接称呼“您”，不使用“您可能更关心”“你应该”“大家需要”等替听众做主的句式。
- 允许表达有依据的判断和与事件直接相关的影响；除公共安全、明确截止时间或官方操作要求外，不向听众下指令。
...
先在内部完成选材，不要输出计划：
1. 为每条事实卡确定“新发生了什么”“经济账涉及谁的收入、成本或价格”“编排计划指定的听众问题如何回答”“听众能采取什么行动或该留意什么”“哪些结论材料并不支持”。
2. 严格按编排计划排序。transition=related/resolve 时只使用 transition_reason 描述的真实关系衔接；transition=reset 时短重置，不因相邻而互相借用事实。
...
- opening.listener_question 非空时，必须自然提出该问题；promise_fact_id 对应正文必须明确回答它，不能改问另一个问题。
- {deep_preview_rule}
...
3. 回答一个听众真正会用到的问题：会影响谁、要花多少钱、何时能用、需要满足什么条件、有什么安全阈值、现在能做什么、还有什么没有定论。
   有编排计划时必须回答该 item.listener_question，并落实 listener_value；不能用“仍需关注”代替答案。
4. 如需提示风险，给出材料支持的具体边界或动作。不要只说“值得关注”“影响深远”“注意风险”。
5. “听众价值”是事件对人的直接影响，不是主持人给听众布置任务。除公共安全、明确截止时间或官方操作要求外，不写“应查看、应核对、建议咨询、需要比较”等行动清单。
6. 不口播编辑过程或证据缺口清单。把“材料没有提供、事实卡没有说明、不能据此判断”改成与事件直接相关的一句边界；如果边界本身不影响听众理解，就删除。
...
## 衔接与口播
- 相邻新闻只在编排计划标为 related/resolve 时按 transition_reason 衔接。direct 直接进入，reset 用一句短重置；共同关键词不等于真实关系。
...
- 不使用“您可能更关心”“对关注这件事的听众来说”等编辑腔，不把资料核验步骤、查阅清单或写作说明念给听众。
...
4. 来源名只在支撑可信度、争议或时效时自然口播一次。不要每句话都以“根据”开头，也不要把证据 URL 写入口播文本。
...
- “听众价值”只写事件直接造成的影响和事实支持的限制。除公共安全、明确截止时间或官方操作要求外，不对听众下指令，不写资料核验清单。
- 不使用“您可能更关心”，不口播“材料没有提供、事实卡没有说明”等编辑过程；确有必要时只保留一句与事件直接相关的结论边界。
