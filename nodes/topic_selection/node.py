...
def _analyze_relevance(
    ...
    logs.append(f"[TopicSelection] Config: {target.masked_summary()}")
    ...
    if not has_llm_runtime_config(config):
        logs.append(
            f"[TopicSelection] No usable LLM runtime ({target.masked_summary()}), skipping AI analysis"
        )
        logs.append(
...
def _llm_batch_analyze(
    contents: list[dict], config: TopicSelectionConfig, client: LLMRuntime, logs: list[str]
) -> list[dict]:
