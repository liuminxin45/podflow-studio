...
class NodeContext:
    ...
    def __init__(self, label: str, state: dict[str, Any]):
        if state.get("schema_version") != SCHEMA_VERSION:
            raise ValueError(
                f"Unsupported episode schema: expected {SCHEMA_VERSION}, got {state.get('schema_version')!r}"
            )
        self.label = label
        self.logs: list[str] = state.get("logs", [])
        self.errors: list[str] = state.get("errors", [])
        self.node_name = _label_to_node_name(label)
        self._initial_node_error_count = self._node_error_count()
        self._t0 = time.time()
    ...
    def log_end(self, detail: str = "") -> None:
        ...
        if detail:
            self.log(detail)
        err_count = self._new_node_error_count()
    ...
    def _node_error_count(self) -> int:
        return sum(
            1
            for error in self.errors
            if isinstance(error, dict) and error.get("node") == self.node_name
        )

    def _new_node_error_count(self) -> int:
        return max(0, self._node_error_count() - self._initial_node_error_count)
    ...
    def finalize(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        err_count = self._new_node_error_count()
        PipelineManifest.record(state, self.node_name, self.elapsed, error_count=err_count)
        return state
