...
桌面端启动后默认进入「今日简报」。完成 AI 成稿目标和声音服务配置后，可以直接：

1. 留空输入框，从已配置的推荐新闻源生成今日节目；
2. 输入关注主题，让系统围绕主题筛选；
3. 每行粘贴一个公开网页链接，或直接粘贴素材文字。

点击「生成今日节目」后，PodFlow 会自动完成素材收集、核验与选题、成稿、配音、
音频装配和机器审核。任务可以在后台运行，完成后首页先展示可播放结果。快速入口
只运行到 Review，不会自动执行 Publish；未通过质量门禁或尚未人工终审的音频会明确
标记为草稿试听，不能正式发布。详细行为见 [今日简报使用说明](docs/quick-briefing.md)。
...
桌面端默认先提供结果导向的「今日简报」入口。需要精细控制时，再打开制作工作台：

1. **今日简报**：输入主题或指定素材，一次发起生成，完成后直接试听。
2. **节目库**：继续收听、恢复制作，并按栏目管理历史节目。
3. **素材**：检查来源和发现结果，必要时重新采集或手动调整。
4. **成稿**：检查事实卡片和节目结构，编辑真正要播出的口播稿。
5. **制作**：为稿件分段生成语音，或替换为真人录音，然后自动装配最终音频。
6. **发布**：检查节目元数据、音频和警告，导出 RSS 与发布包。
