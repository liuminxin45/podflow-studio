...
Runs test modules for all active nodes and reports results.
"""
...
from scripts.verify_nodes import test_node as run_node_cli_smoke
...
def run_node_test(node_name: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            if not run_node_cli_smoke(node_name):
                return False, "Unit script passed, but the real node CLI smoke test failed"
