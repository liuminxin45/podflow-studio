...
from src.topic_selection.models import TopicCandidate, TopicDecision
...
class TopicGate:
    ...
    def gate_topics(
        ...
        self.logger.info(f"LLM Gate: 评估 {len(sorted_candidates)} 个候选主题")
        ...
        for candidate in sorted_candidates:
            ...
            if decision:
                ...
                if decision.should_publish:
                    ...
                    self.logger.info(
                        f"✅ 通过: {candidate.topic_id} "
                        f"(优先级={decision.publish_priority}/5, hook={decision.core_hook[:50]})"
                    )
                else:
                    self.logger.info(f"❌ 拒绝: {candidate.topic_id} (风险={decision.risk})")
    ...
    def _get_system_prompt(self) -> str:
        return """你是播客选题编辑，负责决策哪些主题值得做成播客。
